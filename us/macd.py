...
@click.option('--src-dir', default='fix_amex_daily', help=u'数据目录')
@click.option('--dest-dir', default='amex_macd_daily', help='macd目录')
@print_time
def macd(filename, max_days, src_dir, dest_dir):
    '''
    计算移动平均线
    '''
    print(filename, max_days, src_dir, dest_dir)
    if filename == 'ALL':
    ...
    for filename in filenames:
        df = _macd(filename, max_days, src_dir, dest_dir)
